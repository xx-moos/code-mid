...
export class CommentAdminEditComponent implements OnInit {
  id: number = 0;
  type: string = '';
  ...
  submit() {
    ...
    if (this.editData?.id) {
      this.ApiService.put('/api/admin/announcements/' + this.editData.id, {
      ...
      this.ApiService.post('/api/admin/announcements', {
        ...this.validateForm.value,
      }).subscribe(
        (res) => {
          console.log(res);
